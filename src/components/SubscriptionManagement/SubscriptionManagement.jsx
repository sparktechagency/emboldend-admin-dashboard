...
const SubscriptionManagement = () => {
  ...
  // Helper function to check if plan is monthly
  const isMonthlyPlan = (planNameValue) => {
    return planNameValue && planNameValue.toLowerCase().includes('monthly');
  };
  ...
  const handleCreate = async () => {
    try {
      const createData = {
        ...
        currency,
        isUnionized,
        discount: isMonthlyPlan(planName) ? undefined : discount,
        discountValue: (isMonthlyPlan(planName) || !discountValue) ? undefined : Number(discountValue)
      };
  ...
  // Handle plan name change and clear discount fields for monthly plans
  const handlePlanNameChange = (value) => {
    setPlanName(value);
    if (isMonthlyPlan(value)) {
      setDiscount('');
      setDiscountValue('');
    }
  };
