...
const InstituteManagementTableHead = ({ columns , statusFilter }) => {
        ...
        <div className="border-2 border-opacity-50 rounded-lg bg-surfacePrimary border-primary">
          {
            isLoading ? <div className='h-[200px] flex items-center justify-center'><Spin size='small' /></div> : institutes.length > 0 ? (
              institutes.map((item , idx) => <InstituteManagementTableBody item={item} key={item._id} list={idx + 1 } refetch={refetch} />)
            ) : (
