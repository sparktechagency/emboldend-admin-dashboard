...
const TermsConditions = () => {
  ...
  // API hooks
  const { data: getTerm, isLoading: getTermLoading, refetch } = useGetTermConditionsQuery();
  const [createTerm, { isLoading: TermLoading }] = useCreateTermConditionsMutation();
  ...
  const handleSave = async () => {
    try {
      ...
      if (result.success) {
        message.success(result?.data?.message || "Terms and conditions saved successfully");
        // Refetch the data after successful save
        refetch();
      } else {
        message.error(result?.data?.message || "Failed to save terms and conditions");
      }
    } catch (err) {
      message.error(err?.data?.message || "An error occurred while saving terms and conditions");
